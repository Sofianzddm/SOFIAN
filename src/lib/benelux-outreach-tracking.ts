 ...
 * module Prospection BENELUX.
 *
 * Variante de lib/outreach-tracking.ts : les compteurs sont stockés sur
 * benelux_outreach_touches (id du touch en paramètre).
 */
...
export function decodeBeneluxOutreachUrlParam(encoded: string): string | null {
  try {
...
function rewriteLinks(html: string, touchId: string): string {
  ...
  return html.replace(
    ...
    (match, before: string, quote: string, url: string, after: string) => {
      ...
      const newUrl = `${baseUrl}/api/email/track/benelux-outreach/click?id=${encodeURIComponent(
        touchId
      )}&u=${encoded}`;
...
function buildPixelTag(touchId: string): string {
  const baseUrl = getBaseUrl();
  const src = `${baseUrl}/api/email/track/benelux-outreach/open?id=${encodeURIComponent(touchId)}`;
...
export function injectBeneluxOutreachTracking(html: string, touchId: string): string {
