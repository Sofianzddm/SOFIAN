...
import { checkOutreachBounces } from "@/lib/outreach-bounce-check";

/**
 * POST → vérification rétroactive des bounces sur tous les mails Outreach
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await checkOutreachBounces(body.apply === true);
    return NextResponse.json(result);
  } catch (error) {
    console.error("POST /api/outreach/check-bounces:", error);
