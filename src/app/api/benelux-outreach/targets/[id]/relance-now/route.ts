...
import { executeBeneluxOutreachRelance } from "@/lib/benelux-outreach-send";
import {
...
async function loadLatestSentTouch(targetId: string) {
  return prisma.beneluxOutreachTouch.findFirst({
  ...
  try {
    ...
    const body = applyCastingTemplateVars(
      ...
      ),
      {
        ...
        company: touch.target.companyName || "",
      }
    );
    ...
    console.error("GET /api/benelux-outreach/targets/[id]/relance-now:", error);
...
export async function POST(
  ...
  try {
    ...
    console.error("POST /api/benelux-outreach/targets/[id]/relance-now:", error);
    return NextResponse.json(
