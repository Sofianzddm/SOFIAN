...
// Génère une URL présignée pour upload direct du logo partenaire vers S3.
export async function POST(request: Request) {
  try {
    ...
    const { partnerId, contentType } = await request.json();
    ...
    const key = buildKey(
      "glowup-partners",
      `partner-${partnerId}-${timestamp}.${extFromContentType(ct)}`
    );
