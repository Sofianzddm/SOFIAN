...
import {
  applyAgencyTemplateVars,
  buildAgencyRelanceTemplate,
  buildPartnerLink,
...
async function loadLatestSentTouch(targetId: string) {
  return prisma.agencyOutreachTouch.findFirst({
...
export async function GET(
  ...
  try {
    ...
    const subject = subjectSrc.toLowerCase().startsWith("re:")
      ? subjectSrc
      : `Re: ${subjectSrc}`;
    const body = applyAgencyTemplateVars(
      buildAgencyRelanceTemplate(
      ...
      ),
      {
        ...
        link: buildPartnerLink(touch.target.partnerSlug),
      }
    );
    ...
    console.error("GET /api/agency-outreach/targets/[id]/relance-now:", error);
...
export async function POST(
  ...
  try {
    ...
    console.error("POST /api/agency-outreach/targets/[id]/relance-now:", error);
    return NextResponse.json(
