...
export default function NouveauDevisLibrePage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/documents/${editId}`, { cache: "no-store" })
      .then((r) => {
        if (!r.ok) throw new Error("Erreur chargement devis");
        return r.json();
      })
      .then((doc) => {
      ...
      .catch((e) => {
        console.error(e);
        setError("Impossible de charger le devis à modifier.");
      })
      .finally(() => {
  ...
  const handleSubmit = async (finaliser: boolean) => {
    ...
    try {
      ...
      const res = await fetch(editId ? `/api/devis/standalone/${editId}` : "/api/devis/standalone", {
      ...
      if (id) {
        ...
        router.replace("/factures?tab=quotes");
      }
    } catch (e: any) {
      setError(e.message || "Erreur lors de la sauvegarde du devis");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#1A1110]" style={{ fontFamily: "Spectral, serif" }}>
            {editId ? "Modifier le devis" : "Nouveau devis libre"}
          </h1>
          <p className="text-sm text-gray-500 mt-1">
            {editId
              ? "Modifie un devis non lié à une collaboration, avec le même format que les devis existants."
              : "Crée un devis non lié à une collaboration, avec le même format que les devis existants."}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-8">
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <span className="block text-xs text-gray-500 mt-1">
                  Tous les montants et le PDF seront libellés dans cette devise.
                </span>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
          ...
          <button
            ...
            disabled={submitting}
          >
            {submitting ? "Enregistrement..." : "Finaliser le devis"}
          </button>
        </div>
      </div>
    </div>
